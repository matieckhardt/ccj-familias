...
function RegisterTaller() {
  ...
  const getUserData = async function (res, req) {
    ...
    const alumnos = await fetch(
      "http://localhost:4000/api/v1/Legajos/" + datosMhg[0].CODFAM
    );
    ...
    const cursos = await fetch("http://localhost:4000/api/v1/alumnosCursos");
  ...
  const getTalleres = async function (curso, event) {
    ...
    if (!curso) return;
      ...
      const response = await fetch(
        "http://localhost:4000/api/v1/cupos/find/" + curso
      );
  ...
  const registrados = async function (e) {
    const response = await fetch(
      "http://localhost:4000/api/v1/talleres/registrados/" + e
    );
    const objeto = await response.json();
  ...
  const disponibles = async function (e) {
    const response = await fetch(
      "http://localhost:4000/api/v1/cupos/find/" + e
    );
    const objeto = await response.json();
